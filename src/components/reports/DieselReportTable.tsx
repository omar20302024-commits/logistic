...
export function DieselReportTable({
  rows,
  totalDiesel,
  tripsCount,
  total,
  ...
  const handleExport = () => {
    exportToCsv(
      `تقرير_الديزل_${filters.from}_${filters.to}`,
      ["التاريخ", "رقم الرحلة", "السائق", "الشركة", "مبلغ الديزل"],
      rows.map((r) => [r.trip_date, r.trip_number, r.driver_name, r.company_name, r.diesel_amount])
    );
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white shadow-sm">
        {rows.length === 0 ? (
          <EmptyState icon={Fuel} title="لا توجد بيانات ديزل" />
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-100 text-right text-xs text-zinc-500">
                  ...
                  <th className="px-4 py-3 font-medium">مبلغ الديزل</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-b border-zinc-50 hover:bg-zinc-50/60">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap font-semibold" dir="ltr">{formatCurrency(r.diesel_amount, currencySymbol)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="border-t-2 border-zinc-200 bg-zinc-50 font-bold text-zinc-900">
                  <td colSpan={4} className="px-4 py-3">الإجمالي ({formatNumber(tripsCount)} رحلة)</td>
                  <td className="px-4 py-3 whitespace-nowrap" dir="ltr">{formatCurrency(totalDiesel, currencySymbol)}</td>
                </tr>
              </tfoot>
            </table>
          </div>

...
// ملاحظة أمان: هذه الصفحة لا تستدعي إطلاقاً fn_driver_period_summary أو v_trips_full
// أو أي مصدر يحتوي trip_amount/trip_profit/diesel. تستخدم فقط الدوال الآمنة المخصصة
// لكشف السائق: fn_driver_public_trips و fn_driver_public_summary.
  ...
  if (!driverId) {
    return (
      <div className="flex flex-col gap-6">
        ...
        <EmptyState
          icon={FileText}
          title="لم يتم تحديد سائق"
          description="ارجع لكشف الحساب الداخلي واختر سائقاً أولاً"
        />
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between print:hidden">
        <div>
          ...
          <h1 className="text-xl font-bold text-zinc-900">كشف حساب السائق (نسخة قابلة للمشاركة)</h1>
          <p className="text-sm text-zinc-500">لا تحتوي أي بيانات مالية سرية — آمنة للإرسال للسائق</p>
        </div>
        <PrintClientButton />
      </div>
